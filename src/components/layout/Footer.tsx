...
const supportLinks = [
  ...
  { 
    label: app.lang === "ar" ? "مركز المساعدة" : "Help Center", 
    href: "#", 
    icon: Headphones,
    isSupport: true 
  },
...
return (
  ...
  >
    ...
    {/* ===== شريط الرموز المتحركة في الأسفل (تم وضعه داخل حاوية بخلفية ساطعة وواضحة جداً لتظهر الأيقونات باللون الغامق الداكن #0d2e2a بدون أي اختفاء) ===== */}
    <div className="relative border-t border-emerald-400/40 bg-emerald-100/95 text-[#0d2e2a] py-4 overflow-hidden shadow-2xl">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-emerald-500 to-transparent" />
      <div className="font-extrabold [&_svg]:text-[#0d2e2a] [&_span]:text-[#0d2e2a]">
        <FloatingIconsBar />
      </div>
    </div>
    ...
    {/* ===== ستايلات الحركة (السلايدر والأمواج) ===== */}
    <style>{`
      @keyframes marquee {
