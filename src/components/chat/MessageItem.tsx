...
function LocationPreview({ message }: { message: IMessageItem }) {
  ...
  // استخدام location إذا كان موجوداً، وإلا استخراج من النص
    ...
    <div className="mt-2 space-y-2">
      ...
      <div className="flex gap-2">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex-1 px-3 py-1.5 text-xs text-center bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition-colors flex items-center justify-center gap-1"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex-1 px-3 py-1.5 text-xs text-center bg-green-500 hover:bg-green-600 text-white rounded-xl transition-colors flex items-center justify-center gap-1"
        >
...
function AttachmentPreview({ message }: { message: IMessageItem }) {
  ...
  // ✅ لا تعرض المرفقات للرسائل النصية أو المواقع
  ...
  if (message.type === 'file') {
    ...
    return (
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="mt-2 flex items-center gap-3 p-3 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors group max-w-[280px]"
      >
        <File className="h-8 w-8 text-[#0084ff] shrink-0" />
        ...
        <Download className="h-4 w-4 text-muted-foreground group-hover:text-[#0084ff] transition-colors shrink-0" />
      </a>
    );
  }
  ...
  // ✅ افتراضي
  return (
    <div className="mt-2 p-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-sm text-muted-foreground">
...
function ReplyPreview({ message }: { message: IMessageItem }) {
  ...
  return (
    <div className="mb-1 px-3 py-1.5 bg-[#e4e6eb] dark:bg-[#3a3b4a] rounded-lg border-r-4 border-[#0084ff]">
...
const areEqual = (prevProps: MessageItemProps, nextProps: MessageItemProps) => {
  // ✅ فقط أعد التصيير إذا تغيرت هذه الخصائص
  return (
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isSelected && "bg-[#0084ff]/10 rounded-lg",
        className
      )}
    ...
    >
      ...
      {!isMine && showAvatar && (
        <div className="flex-shrink-0 mt-1">
          <Avatar className="h-8 w-8 ring-2 ring-white dark:ring-[#1a1a2e] shadow-sm">
      ...
      <div className={cn(
        ...
        {!isMine && !isFirstInGroup && (
          <span className="text-xs font-medium text-[#0084ff] mb-0.5">
            {senderName}
          </span>
        )}
        ...
        {/* ===== ✅ فقاعة الرسالة - مثل الماسنجر ===== */}
        <div
          className={cn(
            "relative rounded-2xl px-4 py-2.5 break-words shadow-sm",
            isMine
              ? "bg-[#0084ff] text-white rounded-br-sm"
              : "bg-white dark:bg-[#3a3b4a] text-slate-900 dark:text-white rounded-bl-sm",
            isSelected && "ring-2 ring-[#0084ff]",
        ...
        >
          ...
          {/* القائمة المنسدلة عند التحويم (للمستخدم فقط) */}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-6 w-6 rounded-full bg-white dark:bg-[#242538] shadow-md hover:bg-slate-100 dark:hover:bg-slate-800"
                  >
                ...
                <DropdownMenuContent align="end" className="w-48 rounded-xl p-1 shadow-xl">
                  {!message.is_deleted && onReply && (
                    <DropdownMenuItem className="cursor-pointer gap-2 rounded-lg" onClick={() => onReply(message)}>
                      <Reply className="h-4 w-4" />
                      {app.lang === "ar" ? "رد" : "Reply"}
                    </DropdownMenuItem>
                  )}
                  {!message.is_deleted && onForward && (
                    <DropdownMenuItem className="cursor-pointer gap-2 rounded-lg" onClick={() => onForward(message)}>
                      <Forward className="h-4 w-4" />
                      {app.lang === "ar" ? "إعادة توجيه" : "Forward"}
                    </DropdownMenuItem>
                  )}
                  <DropdownMenuItem className="cursor-pointer gap-2 rounded-lg" onClick={() => {
                  ...
                  {onPin && (
                    <DropdownMenuItem className="cursor-pointer gap-2 rounded-lg" onClick={() => onPin(message.id)}>
...
// ====== نسخة مبسطة (بدون القائمة المنسدلة) ======
        ...
        <div className="flex items-start gap-2 px-4 py-1">
          <div className="flex-shrink-0 mt-1">
            <Avatar className="h-8 w-8">
              <AvatarFallback className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white text-xs animate-pulse">
                ...
              </AvatarFallback>
            </Avatar>
          </div>
          <div className="bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-tl-sm px-4 py-2.5 shadow-sm">
            <div className="flex items-center gap-1">
              ...
              <span className="flex gap-0.5">
                {[0, 1, 2].map((i) => (
                  <span
                    key={i}
                    className="h-1.5 w-1.5 rounded-full bg-[#0084ff] animate-pulse"
