...
<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
  <div>
    ...
    <p className="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-2 flex-wrap">
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#2a655f]/5 border border-[#2a655f]/10 hover:bg-[#2a655f]/10 transition-colors">
        <Building2 className="h-3.5 w-3.5 text-[#2a655f]" />
        <span className="text-[#2a655f] font-medium">{companies.length}</span>
      ...
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-800/30 hover:bg-amber-100/50 transition-colors">
        ...
        <span className="text-amber-600 dark:text-amber-400 font-medium">
          {companies.filter((c: any) => c.is_verified === true).length}
        </span>
        <span className="text-xs text-muted-foreground">{isArabic ? 'موثقة' : 'verified'}</span>
      </span>
    </p>
  </div>
  ...
  <div className="flex items-center gap-2 flex-wrap">
    ...
    <Dialog open={showAddCompany} onOpenChange={setShowAddCompany}>
      ...
      <DialogContent className="max-w-md rounded-2xl border-2 border-pink-400/60 dark:border-pink-400/40 shadow-2xl shadow-pink-500/20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl p-0 overflow-hidden">
        <Button
          variant="ghost"
          size="icon"
          ...
          <form onSubmit={handleAddCompany}>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input name="name_ar" placeholder="شركة التوصيل السريع" required className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
              </div>
              <div className="space-y-2">
                <Label className="text-[#2a655f] dark:text-white">{isArabic ? "اسم الشركة (إنجليزي)" : "Company Name (English)"}</Label>
                <Input name="name_en" placeholder="Fast Delivery Company" className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
              </div>
              <div className="space-y-2">
                <Label className="text-[#2a655f] dark:text-white">{isArabic ? "رقم الهاتف (للمدير)" : "Phone Number (for Manager)"}</Label>
                <Input name="phone" type="tel" placeholder="09XXXXXXXX" required className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
              ...
              <div className="space-y-2">
                ...
                <Input name="password" type="password" placeholder="********" required minLength={6} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                <p className="text-xs text-muted-foreground">
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setShowAddCompany(false)} className="rounded-xl border-2 border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-gray-50/80 dark:hover:bg-gray-700/30 hover:border-slate-400 dark:hover:border-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-all duration-300">
      ...
      {/* ===== COMPANIES GRID - خلفية بيضاء وبوردر وردي ===== */}
  ...
  >
    ...
    {isArabic ? "إضافة شركة" : "Add Company"}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCompanies.map((company: any) => {
            ...
            return (
              ...
              >
                {/* ✅ بوردر متحرك */}
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-pink-400/0 via-pink-400/5 to-pink-400/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-pink-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      {company.logo_url ? (
                        ...
                        <div className="h-12 w-12 rounded-xl bg-[#f9a8d4]/20 dark:bg-[#f9a8d4]/10 flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 border-2 border-pink-400/60 dark:border-pink-400/40">
                ...
                <CardContent className="relative z-10">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-white/60 dark:bg-slate-800/50 rounded-xl p-2.5 text-center group-hover:bg-[#f9a8d4]/20 transition-colors duration-300 border-2 border-pink-400/60 dark:border-pink-400/40">
                  ...
                  {adminUser && (
                    <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground border-t-2 border-pink-400/30 pt-2.5">
                      <div className="h-5 w-5 rounded-full bg-[#f9a8d4]/20 flex items-center justify-center group-hover:scale-110 transition-all duration-300">
                      ...
                      <Badge className="bg-[#f9a8d4]/20 text-[#2a655f] border-2 border-pink-400/60 dark:border-pink-400/40 text-[8px]">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <div className="flex items-center gap-1 flex-1">
                      <div className="h-1.5 w-full max-w-[100px] rounded-full bg-white/60 dark:bg-slate-700 overflow-hidden group-hover:bg-[#f9a8d4]/20 transition-colors duration-300">
                        <div
            ...
            <form onSubmit={handleAddAdminToCompany}>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input name="full_name" placeholder={isArabic ? "أدمن الشركة" : "Company Admin"} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                </div>
                <div className="space-y-2">
              ...
              <DialogFooter>
                <Button type="button" variant="outline" onClick={() => setShowAddAdmin(false)} className="rounded-xl border-2 border-slate-300 text-slate-600 hover:bg-slate-100 hover:border-slate-400 hover:text-slate-800 transition-all duration-300">
                  <X className="h-4 w-4 mr-1" />
                  {isArabic ? "إلغاء" : "Cancel"}
                </Button>
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleAddDistributorToCompany}>
              <div className="space-y-4 py-4">
                ...
                <div className="text-xs text-muted-foreground bg-[#fbcfe8] dark:bg-[#fbcfe8]/20 p-3 rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                  {isArabic 
              ...
              <DialogFooter className="gap-2 pt-4 border-t-2 border-pink-400/30">
                <Button type="button" variant="outline" onClick={() => setShowAddDistributorDialog(false)} className="rounded-xl border-2 border-slate-300 text-slate-600 hover:bg-slate-100 hover:border-slate-400 hover:text-slate-800 transition-all duration-300">
                  <X className="h-4 w-4 mr-1" />
                  {isArabic ? "إلغاء" : "Cancel"}
                </Button>
                <Button 
      ...
      <Dialog open={showEditCompany} onOpenChange={setShowEditCompany}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border-2 border-pink-400/60 dark:border-pink-400/40 shadow-2xl shadow-pink-500/20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl p-0 overflow-hidden">
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-4 end-4 h-8 w-8 rounded-full hover:bg-slate-100 z-20 transition-all duration-300 border-2 border-slate-300"
              ...
              <form onSubmit={handleUpdateCompany}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                  <div className="space-y-2">
                    ...
                    <Input name="name_ar" defaultValue={selectedCompany.name_ar} required className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "الاسم (إنجليزي)" : "Name (English)"} *</Label>
                    <Input name="name_en" defaultValue={selectedCompany.name_en} required className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>

                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "الهاتف" : "Phone"}</Label>
                    <Input name="phone" defaultValue={selectedCompany.phone || ""} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>

                  <div className="space-y-2 md:col-span-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "العنوان (عربي)" : "Address (Arabic)"}</Label>
                    <Input name="address_ar" defaultValue={selectedCompany.address_ar || ""} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2 md:col-span-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "العنوان (إنجليزي)" : "Address (English)"}</Label>
                    <Input name="address_en" defaultValue={selectedCompany.address_en || ""} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>

                  <div className="space-y-2 md:col-span-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "الوصف (عربي)" : "Description (Arabic)"}</Label>
                    <Textarea name="description_ar" defaultValue={selectedCompany.description_ar || ""} rows={2} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2 md:col-span-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "الوصف (إنجليزي)" : "Description (English)"}</Label>
                    <Textarea name="description_en" defaultValue={selectedCompany.description_en || ""} rows={2} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>

                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "السعر الأساسي" : "Base Price"}</Label>
                    <Input name="base_price" type="number" step="0.01" defaultValue={selectedCompany.base_price || 0} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "السعر لكل كم" : "Price per KM"}</Label>
                    <Input name="price_per_km" type="number" step="0.01" defaultValue={selectedCompany.price_per_km || 0} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "الحد الأدنى للتوصيل المجاني" : "Free Delivery Threshold"}</Label>
                    <Input name="free_delivery_threshold" type="number" step="0.01" defaultValue={selectedCompany.free_delivery_threshold || 0} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "متوسط وقت التوصيل (دقيقة)" : "Avg Delivery Time (min)"}</Label>
                    <Input name="avg_delivery_time" type="number" defaultValue={selectedCompany.avg_delivery_time || 60} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "الحد الأدنى للتوصيل" : "Min Delivery Fee"}</Label>
                    <Input name="min_delivery_fee" type="number" step="0.01" defaultValue={selectedCompany.min_delivery_fee || 0} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[#2a655f] dark:text-white">{isArabic ? "الحد الأقصى للتوصيل" : "Max Delivery Fee"}</Label>
                    <Input name="max_delivery_fee" type="number" step="0.01" defaultValue={selectedCompany.max_delivery_fee || 15000} className="border-2 border-pink-400/60 dark:border-pink-400/40 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/30 rounded-xl transition-all duration-300" />
                  </div>

                  <div className="space-y-2 md:col-span-2">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      <div className="flex items-center gap-2 bg-white/50 dark:bg-slate-800/50 p-2 rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                        <input type="checkbox" name="has_tracking" id="has_tracking" defaultChecked={selectedCompany.has_tracking} className="h-4 w-4 rounded border-pink-400/60 text-[#d81b60] focus:ring-pink-500" />
                        <label htmlFor="has_tracking" className="text-sm text-[#2a655f] dark:text-white">{isArabic ? "تتبع" : "Tracking"}</label>
                      </div>
                      <div className="flex items-center gap-2 bg-white/50 dark:bg-slate-800/50 p-2 rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                        <input type="checkbox" name="has_insurance" id="has_insurance" defaultChecked={selectedCompany.has_insurance} className="h-4 w-4 rounded border-pink-400/60 text-[#d81b60] focus:ring-pink-500" />
                        <label htmlFor="has_insurance" className="text-sm text-[#2a655f] dark:text-white">{isArabic ? "تأمين" : "Insurance"}</label>
                      </div>
                      <div className="flex items-center gap-2 bg-white/50 dark:bg-slate-800/50 p-2 rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                        <input type="checkbox" name="has_cod" id="has_cod" defaultChecked={selectedCompany.has_cod} className="h-4 w-4 rounded border-pink-400/60 text-[#d81b60] focus:ring-pink-500" />
                        <label htmlFor="has_cod" className="text-sm text-[#2a655f] dark:text-white">{isArabic ? "الدفع عند الاستلام" : "Cash on Delivery"}</label>
                      </div>
                      <div className="flex items-center gap-2 bg-white/50 dark:bg-slate-800/50 p-2 rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                        <input type="checkbox" name="has_express" id="has_express" defaultChecked={selectedCompany.has_express} className="h-4 w-4 rounded border-pink-400/60 text-[#d81b60] focus:ring-pink-500" />
                  ...
                  <div className="space-y-2 md:col-span-2">
                    <div className="flex items-center gap-2 bg-white/50 dark:bg-slate-800/50 p-3 rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                      <input type="checkbox" name="is_active" id="is_active" defaultChecked={selectedCompany.is_active} className="h-4 w-4 rounded border-pink-400/60 text-[#d81b60] focus:ring-pink-500" />
                ...
                <DialogFooter>
                  <Button type="button" variant="outline" onClick={() => setShowEditCompany(false)} className="rounded-xl border-2 border-slate-300 text-slate-600 hover:bg-slate-100 hover:border-slate-400 hover:text-slate-800 transition-all duration-300">
                    <X className="h-4 w-4 mr-1" />
                    {isArabic ? "إلغاء" : "Cancel"}
                  </Button>
      ...
      <Dialog open={isCompanyDialogOpen} onOpenChange={setIsCompanyDialogOpen}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto rounded-2xl border-2 border-pink-400/60 dark:border-pink-400/40 shadow-2xl shadow-pink-500/20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl p-0 overflow-hidden">
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-4 end-4 h-8 w-8 rounded-full hover:bg-slate-100 z-20 transition-all duration-300 border-2 border-slate-300"
          ...
          {selectedCompany && (
            <div className="p-6">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3">
                  {selectedCompany.logo_url ? (
                    ...
                    <div className="h-10 w-10 rounded-xl bg-[#f9a8d4]/20 dark:bg-[#f9a8d4]/10 flex items-center justify-center border-2 border-pink-400/60 dark:border-pink-400/40">
              ...
              {/* ===== ✅✅✅ COMPANY ADMINS ===== */}
              <div className="mt-4 p-4 bg-white dark:bg-[#1e293b] rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className="bg-[#f9a8d4]/20 text-[#2a655f] border-2 border-pink-400/60 dark:border-pink-400/40 text-[10px]">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {companyAdmins.map((admin: any) => {
                      ...
                      return (
                        ...
                        >
                          <div className="h-8 w-8 rounded-full bg-[#f9a8d4]/20 flex items-center justify-center overflow-hidden shrink-0 border-2 border-pink-400/60 dark:border-pink-400/40">
                            {admin.avatar_url ? (
              ...
              <Tabs value={companyTab} onValueChange={(v) => setCompanyTab(v as any)} className="mt-4">
                <TabsList className="grid w-full grid-cols-3 bg-white dark:bg-[#1e293b] border-2 border-pink-400/60 dark:border-pink-400/40 rounded-xl p-1">
                    ...
                    {getCompanyOrders(selectedCompany.id).length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-80 overflow-y-auto">
                        {getCompanyOrders(selectedCompany.id).slice(0, 20).map((order: any) => (
                          <div key={order.id} className="flex items-center justify-between p-3 bg-white/70 dark:bg-slate-900/70 rounded-lg border-2 border-pink-400/60 dark:border-pink-400/40 hover:shadow-md transition-all hover:border-pink-500">
                            <div className="flex items-center gap-3">
                              <Badge className="bg-[#f9a8d4]/20 text-[#2a655f] border-2 border-pink-400/60 dark:border-pink-400/40 text-[10px] font-mono">
                                #{order.id.slice(0, 8)}
                              </Badge>
                              <Badge className={cn(
                                "border-2 text-[10px]",
                    ...
                    {getCompanyDistributors(selectedCompany.id).length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-80 overflow-y-auto">
                        {getCompanyDistributors(selectedCompany.id).map((dist: any) => (
                          <div key={dist.id} className="flex items-center justify-between p-3 bg-white/70 dark:bg-slate-900/70 rounded-lg border-2 border-pink-400/60 dark:border-pink-400/40 hover:shadow-md transition-all hover:border-pink-500">
                            <div className="flex items-center gap-3">
                              <div className="h-8 w-8 rounded-full bg-[#f9a8d4]/20 flex items-center justify-center border-2 border-pink-400/60 dark:border-pink-400/40">
                ...
                <TabsContent value="analytics" className="mt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-white/70 dark:bg-slate-900/70 rounded-xl border-2 border-pink-400/60 dark:border-pink-400/40">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm p-2 bg-[#fbcfe8]/40 dark:bg-[#fbcfe8]/10 rounded-lg border border-pink-400/60 dark:border-pink-400/40">
                        ...
                        <div className="flex justify-between text-sm p-2 bg-gradient-to-r from-[#2a655f]/10 to-[#f9a8d4]/10 rounded-lg border-2 border-pink-400/60 dark:border-pink-400/40">
